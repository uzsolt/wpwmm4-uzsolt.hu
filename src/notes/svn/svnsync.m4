...
<code>myrepo</code>-t:
_file_highlight(`src/notes/svn/svnsync_init.sh',sh)</p>
<p>Ezután az <code>svnsync</code> parancs segítségével inicializálni kell:
_highlight(`svnsync init --allow-non-empty svn://mirror.server/myrepo svn://already.exist/myrepo',sh)</p>
<p>A szinkronizálás pedig a következő paranccsal történik:
_highlight(`svnsync sync svn://mirror.server/myrepo',sh)</p>
